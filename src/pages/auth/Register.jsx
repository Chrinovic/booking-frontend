...
export default function Register() {
  ...
  const handleRegister = async (e) => {
    e.preventDefault();
    try {
      await axios.post("http://localhost:5000/api/auth/register", { email, password, pseudo });
      navigate("/login");
    } catch (error) {
  ...
  return (
    <div className="flex items-center justify-center h-screen">
      <form onSubmit={handleRegister} className="bg-white p-6 rounded shadow-md">
        ...
        <input type="text" placeholder="Pseudo" className="w-full p-2 border mt-2" onChange={(e) => setPseudo(e.target.value)} />
